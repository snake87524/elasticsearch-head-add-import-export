...
(function( app, i18n, joey ) {
	...
	ui.NodesView = ui.AbstractWidget.extend({
		...
		_import_handler: function(index) {
			let inputObj=document.createElement('input');
			let cluster = this.config.cluster;
			inputObj.addEventListener("click", async () => {
				...
				reader.onload = function(event) {
					const fileContent = event.target.result.replace(new RegExp("\\\"","gm"),"");
					...
					if (primaryKey in metas){
						...
						for(let ii = 1;ii < lines.length; ii++){
							let node = {};
							for(let iii = 0;iii < lines[ii].split(",").length; iii++){ 
								node[metas[iii]] = lines[ii].split(",")[iii];
								requestData.push(node);
							}
						}
						let i = 0;
						let upsertResult = {
							"total":requestData.length,"successful":0,"failed":0
						};
						function _importMul_handler(index, requestData, primaryKey) {
							let keyValue = requestData[i][primaryKey];
							let command = {};
							command["doc"] = requestData[i];
							command["upsert"] = requestData[i];
							cluster.post(encodeURIComponent( index.name ) + "/_doc/" + keyValue + "/_update", JSON.stringify(command), function(r) {
								console.log(r._shards);
								i++;
								upsertResult.successful += r._shards.successful;
								upsertResult.failed += r._shards.failed;
								if(i < requestData.length){
									_importMul_handler(index, requestData, primaryKey);
								}
								else{
									new ui.JsonPanel({
										title: "insert/update",
				...
				new ui.MenuButton({
					...
					menu: new ui.MenuPanel({
						items: [
							...
							{ text: i18n.text("IndexActionsMenu.Import"), onclick: function() { this._import_handler(index); }.bind(this) },
							{ text: i18n.text("IndexActionsMenu.Export"), onclick: function() { this._export_handler(index); }.bind(this) },
	...
	ui.Header = ui.AbstractWidget.extend({
		...
		init: function() {
			...
			var quicks = [
				...
				{ text: i18n.text("Nav.Indexes"), path: "_mapping" },
				{ text: i18n.text("Nav.Download"), path: "_mapping" },
					...
					cluster.get( item.path, function( data ) {
						...
						if(item.text == "导出索引结构"){
							...
							for(let item in data){
								csvString.push([item, "\""+JSON.stringify(data[item]).replace(new RegExp("\"","gm"),"\"\"")+"\""])
							}
							let csvResult = csvString.map(e => e.join(",")).join('\n');
							let link = document.createElement("a")
							let exportContent = '\uFEFF'
